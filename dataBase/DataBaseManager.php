        ...
        foreach ($object->getAllVars() as $key => $var) {
            ...
            if (!empty($columnName = $object->getColumnName($key))) {
                ...
                if (strpos($columnName, "."))
                    ...
                    $resKey = !empty($this->dataBaseHelper->findVarInClasses(new $className, $key)) ? "{$tableName}_{$columnName}" : "{$this->dataBaseHelper->findVarInClasses($whereQuery,$key)->getTableName()}_{$columnName}";

            } else

                $resKey = !empty($this->dataBaseHelper->findVarInClasses(new $className, $key)) ? "{$tableName}_{$key}" : "{$this->dataBaseHelper->findVarInClasses($whereQuery,$key)->getTableName()}_{$key}";
